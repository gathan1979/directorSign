<?php
		...
		$file = $_SERVER['DOCUMENT_ROOT'].'\directorSign\uploads\\'.$row['filename'];     //arxiko arxeio
		$ext = 0;
		if (!file_exists($file)){
		...
		if($path_parts['extension'] == "doc" || $path_parts['extension'] =="docx" || $path_parts['extension'] =="DOC" || $path_parts['extension'] =="DOCX" || $path_parts['extension'] =="xlsx" || $path_parts['extension'] =="xls" || $path_parts['extension'] =="XLSX"){
			...
			if ($ext){
				$convertToPdf = 'OfficeToPDF.exe "'.$_SESSION['externalDiskSignature'].iconv("UTF-8", "UTF-8",$arg1).'" "'.$_SESSION['externalDiskSignature'].iconv("UTF-8", "UTF-8",$arg2).'"';
			}
			else{
			...
			if ($output="" || $output==null){
				...
				echo "4.6 <br>";
				die("Παρουσιάστηκε πρόβλημα στη δημιουργία του PDF");
			}
		}
		else{
		...
		$message = iconv("UTF-8", "UTF-8",'Ακριβές Αντίγραφο Ψηφιακά υπογεγραμμένο από ');
		if ($ext){
			if ($positionSelected){
				$command = 'java -Duser.language=en -jar "C:\Program Files (x86)\JSignPdf\JSignPdf.jar" "'.$_SESSION['externalDiskSignature'].iconv("UTF-8", "UTF-8",$arg2).'" -a  --font-size 10.0 --visible-signature --l2-text "Exact copy digitally signed by ${signer}" -pg '.$page.' -llx '.$llx.' -lly '.$lly.' -urx '.$urx.' -ury '.$ury.' -ka "'.$sigResult['signature'].'" -kst WINDOWS-MY --tsa-server-url https://timestamp.aped.gov.gr/qtss -d "'.$_SESSION['externalDiskSignature'].'"';
			}
			else{
				$command = 'java -Duser.language=en -jar "C:\Program Files (x86)\JSignPdf\JSignPdf.jar" "'.$_SESSION['externalDiskSignature'].iconv("UTF-8", "UTF-8",$arg2).'" -a  --font-size 10.0 --visible-signature --l2-text "Exact copy digitally signed by ${signer}" -llx 335 -lly 780 -urx 565 -ury 830 -ka "'.$sigResult['signature'].'" -kst WINDOWS-MY  --tsa-server-url https://timestamp.aped.gov.gr/qtss -d "'.$_SESSION['externalDiskSignature'].'"';
			}
		}
		else{
			if ($positionSelected){
				//$command = 'java -Duser.language=en -jar "C:\Program Files (x86)\JSignPdf\JSignPdf.jar" "c:\xampp\htdocs\directorSign\uploads\\'.iconv("UTF-8", "UTF-8",$arg2).'" -a  --font-size 10.0 --visible-signature --l2-text "'.iconv("UTF-8", "UTF-8",'Ακριβές Αντίγραφο Ψηφιακά υπογεγραμμένο από ${signer}').'" -pg '.$page.' -llx '.$llx.' -lly '.$lly.' -urx '.$urx.' -ury '.$ury.' -ka "'.$sigResult['signature'].'" -kst WINDOWS-MY  --tsa-server-url https://timestamp.aped.gov.gr/qtss  -d "c:\xampp\htdocs\directorSign\uploads"';
				$command = 'java -Duser.language=en -jar "'.$_SESSION['jsignpdf_jar'].'" "c:\xampp\htdocs\directorSign\uploads\\'.iconv("UTF-8", "UTF-8",$arg2).'" -a  --font-size 10.0 --visible-signature --l2-text "Exact copy digitally signed by ${signer}" -pg '.$page.' -llx '.$llx.' -lly '.$lly.' -urx '.$urx.' -ury '.$ury.' -ka "'.$sigResult['signature'].'" -kst WINDOWS-MY  --tsa-server-url https://timestamp.aped.gov.gr/qtss  -d "c:\xampp\htdocs\directorSign\uploads"';

			}
			else{
				$command = 'java -Duser.language=en -jar "'.$_SESSION['jsignpdf_jar'].'" "c:\xampp\htdocs\directorSign\uploads\\'.iconv("UTF-8", "UTF-8",$arg2).'" -a  --font-size 10.0 --visible-signature --l2-text "Exact copy digitally signed by ${signer}" -llx 335 -lly 780 -urx 565 -ury 830 -ka "'.$sigResult['signature'].'" -kst WINDOWS-MY  --tsa-server-url https://timestamp.aped.gov.gr/qtss  -d "c:\xampp\htdocs\directorSign\uploads"';
		...
		if (strpos(end($output2), 'succesfully') !== false){
			...
			echo $arg3." ====".$arg4."====<br>";
			if ($ext){
				copy($_SESSION['externalDiskSignature'].iconv("UTF-8", "UTF-8",$arg3),$_SESSION['externalDiskSignature'].iconv("UTF-8", "UTF-8",$arg4));
			}
			else{
				copy("uploads\\".iconv("UTF-8", "UTF-8",$arg3),"uploads\\".iconv("UTF-8", "UTF-8",$arg4));
			}
			echo "7";
	   ...
	   // header('Location: headmaster1.php');