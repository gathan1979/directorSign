		...
		<div id="container" class="container-fluid">
			...
			<span class="label label-default">Έγγραφα για ψηφιακή υπογραφή</span><br><br>
				...
				<div id="demo" class="collapse">
					...
					<br><br>
					<input type="button" class="btn btn-primary ektos" id="uploadFileButton" onclick="uploadfile()" value="Μεταφόρτωση Αρχείου"/>
				</div>
			<hr>
					  ...
					  while ($row = mysqli_fetch_array($rslt, MYSQLI_BOTH)){
							...
							if ($row1!=NULL){ 
								...
								if ($row1['nextLevel']<4 && $row1['nextLevel']>-2){ //den exei parei ipografi kai den aporifthike 
									...
									echo '<a href="\directorSign\uploads\\'.$row['filename'].'" target="_blank">'.$row['filename'].'</a> &nbsp&nbsp <span class="badge">'.$newDocs.'</span>&nbsp<i  class="fas fa-search" title="Προεπισκόπηση αρχικού εγγράφου" onclick="preview_PDF(\''.$preview_file.'\');"></i></td><td class="text-right">';
