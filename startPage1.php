...
<html>
	<head>
		...
		<?php
			...
			$notifications='"';			
			$sqlN = "select * from `filestobesigned` where revisionId in (SELECT aa FROM `filestobesigned` WHERE `userId`=".$_SESSION['aa_user'].') and `date`>(select max(notificationUpdate) from lastnotification where userId='.$_SESSION['aa_user'].') order by aa desc limit 5';
			//echo $sqlN;
