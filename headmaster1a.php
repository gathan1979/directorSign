			...
			<div class="table-responsive" id="soma">
				...
				<?php	  
						...
						//$sql = "SELECT * from filestobesigned where revisionId=0 and nextlevel!=2 and userId in
						 ...
						 $sql="SELECT * from filestobesigned where revisionId=0 and nextlevel!=2 and userId in (select attendanceId from signpasswords where department=".$_SESSION['department'].") 
							 and aa not in ( select revisionId from `filestobesigned` where aa in ( SELECT max(aa) FROM `filestobesigned` WHERE `revisionId`!=0 group by revisionId order by `aa` asc) and nextLevel!=1)";
						...
						while ($row = mysqli_fetch_array($rslt, MYSQLI_BOTH)){
							...
							if ($row1!=NULL){      //arxeio me allages
								...
								if($row1['nextLevel']==1){
									...
									echo '<a href="\directorSign\uploads\\'.$row['filename'].'" target="_blank">'.$row['filename'].'</a>&nbsp<i  class="fas fa-search" title="Προεπισκόπηση τελευταίας τροποποίησης" onclick="preview_PDF(\''.$preview_file_last.'\');"></i>&nbsp&nbsp&nbsp-&nbsp&nbsp&nbsp<i class="fab fa-amilia fa-1x" title="Προεπισκόπηση αρχικής ανάρτησης" onclick="preview_PDF(\''.$preview_file.'\');"></i>&nbsp</td><td class="text-right">';
									...
									if ($_SESSION['accessLevel']>="1"){	
										echo "<a class='showModal' href='http://".$_SERVER['SERVER_ADDR']."/directorSign/forward1.php?aa=".$row['aa']."'><span class='glyphicon glyphicon-tag' style='font-size: 20px;' aria-hidden='true' data-toggle='tooltip' title='Ψηφιακή Υπογραφή και Αυτόματη Προώθηση'></a>&nbsp&nbsp&nbsp&nbsp";
								...
								echo '<a href="\directorSign\uploads\\'.$row['filename'].'" target="_blank">'.$row['filename'].'</a>&nbsp<i  class="fas fa-search" onclick="preview_PDF(\''.$preview_file.'\');"></i></td><td class="text-right">';
								echo $row['date'].'</td><td class="text-right">';
								echo $row2['fullName'].'</td><td class="text-right">';
								echo $row['comments'].'</td><td class="text-right">';
								echo "<a href='http://".$_SERVER['SERVER_ADDR']."/directorSign/history.php?aa=".$row['aa']."'><span class='glyphicon glyphicon-inbox' style='font-size: 20px;' aria-hidden='true'  data-toggle='tooltip' title='Προβολή Ιστορικού'></a>&nbsp&nbsp";
								if ($_SESSION['accessLevel']>="1"){		
									echo "<a class='showModal' href='http://".$_SERVER['SERVER_ADDR']."/directorSign/forward1.php?aa=".$row['aa']."'><span class='glyphicon glyphicon-tag' style='font-size: 20px;' aria-hidden='true' data-toggle='tooltip' title='Ψηφιακή Υπογραφή και Αυτόματη Προώθηση'></a>&nbsp&nbsp&nbsp&nbsp";
									echo "<a href='http://".$_SERVER['SERVER_ADDR']."/directorSign/backward.php?aa=".$row['aa']."'><span class='glyphicon glyphicon-download' style='font-size: 20px;' aria-hidden='true' data-toggle='tooltip' title='Αυτόματη Επιστροφή'></a>&nbsp&nbsp&nbsp&nbsp";
									echo "<a class='cancelModal' href='http://".$_SERVER['SERVER_ADDR']."/directorSign/reject.php?aa=".$row['aa']."'><span class='glyphicon glyphicon-ban-circle' style='font-size: 20px;' aria-hidden='true' data-toggle='tooltip' title='Οριστική Απόρριψη'></a></td></tr>";
								}
							}
						}
				?>	  
