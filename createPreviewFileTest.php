<?php
		...
		$type = pathinfo($fileNames[$i],PATHINFO_EXTENSION);
		//$msg.= " τύπος =".$type." "; 
		if($type == "doc" || $type =="docx" || $type =="DOC" || $type =="DOCX" || $type =="xlsx" || $type =="xls" || $type =="XLSX"){
			...
			//allagi stis 15-05-2020 logo bug stin epilogi arxeiou
			//$convertToPdf = 'OfficeToPDF.exe "uploads\\'.iconv("UTF-8", "UTF-8",basename($_FILES["selectedFile".$i]["name"])).'" "uploads\\'.iconv("UTF-8", "UTF-8",pathinfo($target_file,PATHINFO_FILENAME).".pdf").'"';
			//$convertToPdf = 'OfficeToPDF.exe "uploads\\'.iconv("UTF-8", "UTF-8",pathinfo($target_file,PATHINFO_FILENAME).".".pathinfo($target_file,PATHINFO_EXTENSION )).'" "uploads\\'.iconv("UTF-8", "UTF-8",pathinfo($target_file,PATHINFO_FILENAME).".pdf").'"';
			$convertToPdf = 'OfficeToPDF.exe "uploads\\'.iconv("UTF-8", "UTF-8",pathinfo($fileNames[$i],PATHINFO_FILENAME).".".pathinfo($fileNames[$i],PATHINFO_EXTENSION )).'" "uploads\\'.iconv("UTF-8", "UTF-8",pathinfo($fileNames[$i],PATHINFO_FILENAME).".pdf").'"';
