			...
			<div class="container-fluid">
				<div class="navbar-header">
				 ...
				 <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
					...
					<li class="text-center" id="emfanisiOnomatos" style="padding-left:100px;"><a><b>Χρήστης</br></br></b><h6><?php echo $_SESSION['user'];?></h6></a></li>
			...
			<div class="table-responsive" id="soma">
				...
				<?php	  
						...
						//$sql = "SELECT * from filestobesigned where revisionId=0 and nextlevel!=2 and userId in
						 ...
						 $sql="SELECT * from filestobesigned where revisionId=0 and nextlevel!=2 and userId in (select attendanceId from signpasswords where prime=1 and department=".$_SESSION['department'].") 
							 and aa not in ( select revisionId from `filestobesigned` where aa in ( SELECT max(aa) FROM `filestobesigned` WHERE `revisionId`!=0 group by revisionId order by `aa` asc) and (nextLevel>3 or nextLevel<0 ))";
						...
						while ($row = mysqli_fetch_array($rslt, MYSQLI_BOTH)){
							...
							//echo $sql2;
							if ($row1!=NULL){      //arxeio me allages - oxi nea eisagogi
								//preview last upload
								...
								if($row1['nextLevel']==1){
									...
									if ($_SESSION['accessLevel']>="1"){	
										echo "<a class='showModal' href='http://".$_SERVER['SERVER_ADDR']."/directorSign/forward.php?aa=".$row['aa']."'><span class='glyphicon glyphicon-tag' style='font-size: 20px;' aria-hidden='true' data-toggle='tooltip' title='Ψηφιακή Υπογραφή και Αυτόματη Προώθηση'></a>&nbsp&nbsp&nbsp&nbsp";
