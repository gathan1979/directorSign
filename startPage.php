		...
		<div id="container" class="container-fluid" style="background-color:#cadfc0;padding-bottom: 2em;">
			...
			<div class="table-responsive" id="soma">
				...
				<?php
					  ...
					  // to proto tmima tou union epilegei ta eggrafa pou exoun proxorisei epipedo, to deytero tis nees eisagoges
					  $sql = "select * from filestobesigned where aa in (select revisionId from`filestobesigned` \n"
						. "where aa in (SELECT max(aa) FROM `filestobesigned` WHERE `revisionId`!=0 group by \n"
						. "revisionId order by `aa` asc) and nextLevel<4 and nextLevel>=0 ) \n"
						. "and userId=".$_SESSION['aa_user']."\n"
						. "union\n"   
						. "select * from filestobesigned where nextLevel=1 and `revisionId`=0 and userId=".$_SESSION['aa_user']."\n"
						. "and aa not in (select revisionId from `filestobesigned` where aa in \n"
						. "( SELECT max(aa) FROM `filestobesigned` WHERE `revisionId`!=0 \n"
						. "group by revisionId order by `aa` asc))";
						//echo $sql;
					  ...
					  while ($row = mysqli_fetch_array($rslt, MYSQLI_BOTH)){
							...
							//------
							if ($row1!=NULL){   //iparxei anafora sto arxiko eggrafo meso tou revisionId. Den einai nea eisagogi
								...
								if ($row1['nextLevel']<4 && $row1['nextLevel']>-2){ //den exei parei ipografi kai den aporifthike 
									if ($row1['nextLevel']==0){                     //epistrafike apo proistameno
										...
										echo '<tr class="warning"><td class="text-right"><i class="fas fa-flag" title="Προεπισκόπηση τελευταίας ανακοινοποίησης" onclick="preview_PDF(\''.$preview_file_last.'\','.$row['aa'].');"></i>&nbsp ';			    //egkrisi apo proistameno
									} 
									...
									//echo '<a href="\directorSign\uploads\\'.$row['filename'].'" target="_blank">'.$row['filename'].'</a> &nbsp&nbsp <i  class="fas fa-search" title="Προεπισκόπηση αρχικού εγγράφου" onclick="preview_PDF(\''.$preview_file.'\','.$row['aa'].');"></i>'.$relevantDocsElement.'</td><td class="text-right">';
									echo '<button class="btn btn-success" onclick="preview_file_by_name(\''.$row['filename'].'\')">'.$row['filename'].'</button> &nbsp&nbsp <i  class="fas fa-search" title="Προεπισκόπηση αρχικού εγγράφου" onclick="preview_PDF(\''.$preview_file.'\','.$row['aa'].');"></i>'.$relevantDocsElement.'</td><td class="text-right">';
									echo $row['date'].'</td><td class="text-right">';
									
									$levels = allLevelsUp($_SESSION['department']);
									...
									for($i=$row1['nextLevel'];$i<4;$i++){
										echo '<i class="btn btn-danger btn-sm  fas fa-calendar-times fa-2x" ></i>&nbsp';
									}
									/*
									if ($row1['nextLevel']==0){  		//epistrafike apo proistameno
									...
									*/
									...
									echo "<a class='btn btn-info btn-sm' href='http://".$_SERVER['SERVER_ADDR']."/directorSign/history.php?aa=".$row['aa']."'><span class='glyphicon glyphicon-inbox' style='font-size: 20px;' aria-hidden='true' data-toggle='tooltip' title='Προβολή Ιστορικού'></a>&nbsp&nbsp";
									if ($row1['nextLevel']==0){
										echo "<a class='btn btn-danger btn-sm' href='http://".$_SERVER['SERVER_ADDR']."/directorSign/reject.php?aa=".$row['aa']."'><span class='glyphicon glyphicon-ban-circle' style='font-size: 20px;' aria-hidden='true' data-toggle='tooltip' title='Οριστική Απόρριψη'></a>";
									}
									// (-) 15-01-2020
