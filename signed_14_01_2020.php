				...
				<?php	  
					...
					*/
					...
					if($_SESSION['accessLevel']<1){					  
						...
						$sql = "select * from `filestobesigned` where aa in (select revisionId from `filestobesigned` where aa in ( SELECT max(aa) FROM `filestobesigned` WHERE `revisionId`!=0 group by revisionId order by `aa` asc) and nextLevel=4)";
					}
