			...
			<!--<input  type="date" name="imera" id="imera" required="true" onchange="paronProsopiko();"/>-->
				...
				<?php	  
					  ...
					  $sql = "select * from filestobesigned where aa in (select revisionId from `filestobesigned` where aa in ( SELECT max(aa) FROM `filestobesigned` 
								WHERE `revisionId`!=0 group by revisionId order by `aa` asc) and nextLevel=3)";
					  ...
					  while ($row = mysqli_fetch_array($rslt, MYSQLI_BOTH)){
							...
							echo '<button class="btn btn-success btn-sm" onclick="preview_file_by_name(\''.$row['filename'].'\')">'.$row['filename'].'</button>&nbsp<i  class="fas fa-search-plus" title="προεπισκόπηση τελευταίας τροποποίησης εγγράφου" onclick="preview_PDF(\''.$preview_file_last.'\','.$row['aa'].');"></i>&nbsp&nbsp&nbsp-&nbsp&nbsp&nbsp<i  class="fas fa-book" title="προεπισκόπηση αρχικού εγγράφου" onclick="preview_PDF(\''.$preview_file.'\','.$row['aa'].');"></i>'.$relevantDocsElement.'</td><td class="text-right">';
							...
							if ($_SESSION['accessLevel']>"1"){	
								echo "<a class='btn btn-success showModal' href='http://".$_SERVER['SERVER_ADDR']."/directorSign/signature.php?aa=".$row['aa']."'><span class='glyphicon glyphicon-tag' style='font-size: 20px;' aria-hidden='true' data-toggle='tooltip' title='Ψηφιακή Υπογραφή'></a>&nbsp&nbsp";
