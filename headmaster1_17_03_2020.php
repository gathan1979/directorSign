				...
				<div class="navbar-header">
				 ...
				 <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
					...
					<ul class="nav navbar-nav">
						<li class="active text-center"><a href="<?php echo "http://".$_SERVER['SERVER_ADDR']."/directorSign"?>"><span class="label label-warning">Προς Υπογραφή</span></br></br><i class="far fa-file fa-lg"></i></a></li>
						<li class="text-center"><a href="<?php echo "http://".$_SERVER['SERVER_ADDR']."/directorSign/signed.php"?>"><span class="label label-success">Υπογεγραμμένα</span></br></br><i class="far fa-file-pdf fa-lg"></i></a></li>
						<!--<li class="text-center"><a href="<?php echo "http://".$_SERVER['SERVER_ADDR']."/directorSign/rejected.php"?>"><span class="label label-danger">Απορριφθέντα</span></br></br><i class="far fa-file-excel fa-lg"></i></a></li>-->
						<?php 	
								if ($_SESSION['privilege'] == 1){
						...
						?>">
						<span class="label label-primary">Πρωτόκολλο</span></br></br><i class="fab fa-readme fa-lg"></i></a></li>
						...
						Δωράκι</span></br></br><i class="fas fa-gifts fa-lg"></i></a></li>
			...
			<div class="table-responsive" id="soma">
				...
				<?php	  
						...
						//$sql = "SELECT * from filestobesigned where revisionId=0 and nextlevel!=2 and userId in
						 ...
						 $sql="SELECT * from filestobesigned where revisionId=0 and nextlevel!=2 
							...
							if ($row1!=NULL){      // oxi nea eisagogi apo proistameno
								...
								if ($numRows3>0){
									...
									if ($file_extension=="pdf" || $file_extension=="PDF"){
										...
										$preview_file_last = "preview_".$file_without_extension.".pdf";
									}
								}
								...
								if($row1['nextLevel']==1){
									...
									if ($preview_file_last==""){
										echo '<a href="\directorSign\uploads\\'.$row['filename'].'" target="_blank">'.$row['filename'].'</a>&nbsp<i class="fab fa-amilia fa-1x" title="Προεπισκόπηση αρχικής ανάρτησης" onclick="preview_PDF(\''.$preview_file.'\');"></i>'.$relevantDocsElement.'</td><td class="text-right">';
									}
									else{
										echo '<a href="\directorSign\uploads\\'.$row['filename'].'" target="_blank">'.$row['filename'].'</a>&nbsp<i  class="fas fa-search-plus" title="Προεπισκόπηση τελευταίας τροποποίησης" onclick="preview_PDF(\''.$preview_file_last.'\');"></i>&nbsp&nbsp&nbsp-&nbsp&nbsp&nbsp<i class="fab fa-amilia fa-1x" title="Προεπισκόπηση αρχικής ανάρτησης" onclick="preview_PDF(\''.$preview_file.'\');"></i>'.$relevantDocsElement.'</td><td class="text-right">';
									}
									...
									if ($_SESSION['accessLevel']>="1"){	
										if ($_SESSION['device']=="" && $row['aped']){
											echo "r".$row['aped']."<a class='btn btn-success btn-sm showModal' href=''><span class='glyphicon glyphicon-tag' style='font-size: 20px;' aria-hidden='true' data-toggle='tooltip' title='Ψηφιακή Υπογραφή και Αυτόματη Προώθηση'></a>&nbsp&nbsp";
										}
										else{
											echo "r".$row['aped']."<a class='btn btn-success btn-sm showModal' href='http://".$_SERVER['SERVER_ADDR']."/directorSign/forward.php?aa=".$row['aa']."'><span class='glyphicon glyphicon-tag' style='font-size: 20px;' aria-hidden='true' data-toggle='tooltip' title='Ψηφιακή Υπογραφή και Αυτόματη Προώθηση'></a>&nbsp&nbsp";
										}
								...
								echo '<a href="\directorSign\uploads\\'.$row['filename'].'" target="_blank">'.$row['filename'].'</a>&nbsp<i  class="fas fa-search-plus" onclick="preview_PDF(\''.$preview_file.'\');"></i>'.$relevantDocsElement." ".($row['aped'] ? "<img src='images/ermis1.png'/>"  : "").'</td><td class="text-right">';
								// -->
								echo $row['date'].'</td><td class="text-right">';
								echo $row2['fullName'].'</td><td class="text-right">';
								echo $row['comments'].'</td><td class="text-right">';
								...
								if ($_SESSION['accessLevel']>="1"){	
									if ($_SESSION['device']=="" && $row['aped']){
										echo "p".$row['aped']."<a class='btn btn-success btn-sm showModal' href=''><span class='glyphicon glyphicon-tag' style='font-size: 20px;' aria-hidden='true' data-toggle='tooltip' title='Ψηφιακή Υπογραφή και Αυτόματη Προώθηση'></a>&nbsp&nbsp";
									}
									else{
										echo  "p".$row['aped']."<a class='btn btn-success btn-sm showModal' href='http://".$_SERVER['SERVER_ADDR']."/directorSign/forward.php?aa=".$row['aa']."'><span class='glyphicon glyphicon-tag' style='font-size: 20px;' aria-hidden='true' data-toggle='tooltip' title='Ψηφιακή Υπογραφή και Αυτόματη Προώθηση'></a>&nbsp&nbsp";
									}
									echo "<a class='btn btn-warning btn-sm ' href='http://".$_SERVER['SERVER_ADDR']."/directorSign/backward.php?aa=".$row['aa']."'><span class='glyphicon glyphicon-download' style='font-size: 20px;' aria-hidden='true' data-toggle='tooltip' title='Αυτόματη Επιστροφή'></a>&nbsp&nbsp";
